...
(function () {
  ...
  function renderTsrScorecard(stages) {
    ...
    for (var i = 0; i < stages.length; i++) {
      ...
      html +=
        ...
        '<span class="sc-stage-icon">' +
        s.icon +
        '</span>' +
        '<span class="sc-stage-name">' +
        _escapeHtml(s.name) +
        '</span>' +
  ...
  async function renderTsrHome() {
    ...
    if (scScore) {
      scScore.textContent =
        '\u2b50 ' + sc.overall.toFixed(1) + '/5';
    }
    renderTsrScorecard(sc.stages);
